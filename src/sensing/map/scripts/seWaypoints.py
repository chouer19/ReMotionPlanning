...
def getSparse(n0,n1):
    ...
    sparseNum = math.floor(distance / 9)
    if sparseNum == 0:
        return [n0.X()] , [n0.Y()]
    ...
    for i in range(0,int(sparseNum)):
        X = n0.X() + i/sparseNum * (n1.X() - n0.X()) + round(np.random.random() * 3 - 1.5,2)
        Y = n0.Y() + i/sparseNum * (n1.Y() - n0.Y()) + round(np.random.random() * 3 - 1.5,2)
        Xs = Xs + [X]
        Ys = Ys + [Y]
    return Xs,Ys
    pass
...
def main(args):
    ...
    waypointsDir = '/home/chouer/workspace/expr2/ReMotionPlanning/src/data/seWaypoints/'
    waypointsNum = 0
    
    #render = Render(500, 500)
    while waypointsNum < 1000000:
    #while waypointsNum <10:
        #time.sleep(0.5)
        for i in range(0,10):
            newNode = getN2(topuInfo, n0, n1)
